...
export default function ProductCard({ id, name, price, image, category, isCombo, onAdd }: Props) {
  ...
  return (
    <div
      className="group relative bg-white border border-slate-200 rounded-md overflow-hidden hover:border-emerald-400 transition-colors cursor-pointer"
      onClick={() => onAdd(id)}
    >
      ...
      <div className="p-2.5 space-y-1">
        <h3 className="text-sm font-medium text-slate-800 leading-tight line-clamp-1">{name}</h3>
        <p className="text-xs text-slate-400">{category}</p>
        <p className="text-base font-bold text-emerald-700">{fmt(price)}</p>
      </div>
    </div>
  );
}
