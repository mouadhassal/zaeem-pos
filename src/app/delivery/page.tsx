...
const STATUS_BADGE: Record<string, { class: string; label: string }> = {
  AVAILABLE: { class: "bg-emerald-100 text-emerald-700", label: "متاح" },
  BUSY: { class: "bg-amber-100 text-amber-700", label: "مشغول" },
  OFFLINE: { class: "bg-slate-100 text-slate-500", label: "غير متصل" },
  ...
  DELIVERED: { class: "bg-emerald-100 text-emerald-700", label: "تم التوصيل" },
  FAILED: { class: "bg-red-100 text-red-600", label: "فشل" },
  CANCELLED: { class: "bg-slate-100 text-slate-500", label: "ملغي" },
};
    ...
    <div className="h-full flex flex-col" dir="rtl">
      <div className="h-14 shrink-0 flex items-center justify-between px-6 border-b border-slate-200 bg-white">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-lg text-slate-800">إدارة التوصيل</h1>
        </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm transition-all ${
                  tab === t.id ? "bg-white text-emerald-700 font-medium shadow-sm" : "text-slate-500 hover:text-slate-700"
                }`}
              >
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-slate-700">التوصيلات النشطة ({deliveries.length})</h2>
        <button onClick={onRefresh} className="text-sm text-emerald-600 hover:text-emerald-700">
          تحديث
        </button>
      </div>
      <div className="grid gap-3">
        {deliveries.map((d) => {
          ...
          return (
            <div key={d.log_id} className="bg-white rounded-lg border border-slate-200 p-4 space-y-3">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-slate-800">{d.customer_name || "بدون اسم"}</span>
                ...
                <div className="text-left">
                  <div className="font-bold text-emerald-600">{fmt(d.total_cents)}</div>
                  <div className="text-xs text-slate-400">{formatDate(d.assigned_at)} {formatTime(d.assigned_at)}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 p-2 bg-slate-50 rounded-md">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-700">{d.driver_name}</p>
              ...
              {d.delivery_status === "IN_TRANSIT" && (
                <div className="flex gap-2">
                  <button onClick={() => onUpdateStatus(d.log_id, "DELIVERED")} className="flex-1 bg-emerald-500 text-white rounded-md py-2 text-sm font-medium hover:bg-emerald-600 transition-colors">
                    تم التوصيل
                  </button>
                  <button onClick={() => onUpdateStatus(d.log_id, "FAILED")} className="px-3 text-red-500 hover:text-red-600 text-sm">
                    فشل
...
function DriversView({
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-slate-700">السائقين ({drivers.length})</h2>
        <button onClick={onAdd} className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500 text-white rounded-md text-sm font-medium hover:bg-emerald-600 transition-colors">
      ...
      <div className="grid gap-3">
        {drivers.map((d) => {
          ...
          return (
            <div key={d.id} className="bg-white rounded-lg border border-slate-200 p-4 flex items-center gap-4">
              ...
              <button onClick={() => onEdit(d)} className="text-sm text-emerald-600 hover:text-emerald-700 shrink-0">
                تعديل
              </button>
            </div>
          );
...
function DriverForm({ editing, onClose, onSaved }: { editing: DriverRow | null; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-emerald-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-slate-700">{editing ? "تعديل سائق" : "إضافة سائق جديد"}</h3>
        <button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X className="w-4 h-4" /></button>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          <label className="text-xs text-slate-500 font-medium">الاسم</label>
          <input value={name} onChange={(e) => setName(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" placeholder="اسم السائق" />
        </div>
        <div className="space-y-1">
          <label className="text-xs text-slate-500 font-medium">رقم الجوال</label>
          <input value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" placeholder="05xxxxxxxx" dir="ltr" />
        </div>
        <div className="space-y-1">
          <label className="text-xs text-slate-500 font-medium">نوع المركبة</label>
          <select value={vehicleType} onChange={(e) => setVehicleType(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 bg-white">
        ...
        <div className="space-y-1">
          <label className="text-xs text-slate-500 font-medium">لوحة المركبة</label>
          <input value={vehiclePlate} onChange={(e) => setVehiclePlate(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" placeholder="أ ب 1234" />
        </div>
        <div className="space-y-1">
          <label className="text-xs text-slate-500 font-medium">رقم الترخيص</label>
          <input value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" placeholder="رقم رخصة القيادة" />
        </div>
      </div>
      <div className="flex justify-end gap-2 pt-1">
        <button onClick={onClose} className="px-4 py-2 text-sm text-slate-500 hover:text-slate-700">إلغاء</button>
        <button onClick={handleSave} disabled={saving || !name.trim() || !phone.trim()} className="px-4 py-2 bg-emerald-500 text-white rounded-md text-sm font-medium hover:bg-emerald-600 disabled:opacity-50 transition-colors">
...
function ZonesView({
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-slate-700">مناطق التوصيل ({zones.length})</h2>
        <button onClick={onAdd} className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500 text-white rounded-md text-sm font-medium hover:bg-emerald-600 transition-colors">
          <Plus className="w-4 h-4" />
          إضافة منطقة
        </button>
      ...
      <div className="grid gap-3">
        {zones.map((z) => (
          <div key={z.id} className="bg-white rounded-lg border border-slate-200 p-4 flex items-center justify-between">
            <div className="space-y-1">
            ...
            <button onClick={() => onEdit(z)} className="text-sm text-emerald-600 hover:text-emerald-700">تعديل</button>
          </div>
        ))}
        {zones.length === 0 && (
          <div className="text-center py-10 text-slate-400">
...
function ZoneForm({ editing, onClose, onSaved }: { editing: ZoneRow | null; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-emerald-200 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-slate-700">{editing ? "تعديل منطقة" : "إضافة منطقة جديدة"}</h3>
        <button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X className="w-4 h-4" /></button>
      </div>
      <div className="grid grid-cols-3 gap-3">
        <div className="space-y-1">
          <label className="text-xs text-slate-500 font-medium">اسم المنطقة</label>
          <input value={name} onChange={(e) => setName(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" placeholder="مثال: حي النزهة" />
        </div>
        <div className="space-y-1">
          <label className="text-xs text-slate-500 font-medium">رسوم التوصيل (ريال)</label>
          <input value={feeCents} onChange={(e) => setFeeCents(e.target.value)} type="number" min="0" step="0.5" className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" dir="ltr" />
        </div>
        <div className="space-y-1">
          <label className="text-xs text-slate-500 font-medium">أقل قيمة للطلب (ريال)</label>
          <input value={minOrderCents} onChange={(e) => setMinOrderCents(e.target.value)} type="number" min="0" step="5" className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" dir="ltr" />
        </div>
        <div className="space-y-1">
          <label className="text-xs text-slate-500 font-medium">الوقت المقدر (دقيقة)</label>
          <input value={estimatedMinutes} onChange={(e) => setEstimatedMinutes(e.target.value)} type="number" min="5" max="180" className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400" dir="ltr" />
        </div>
      </div>
      <div className="flex justify-end gap-2 pt-1">
        <button onClick={onClose} className="px-4 py-2 text-sm text-slate-500 hover:text-slate-700">إلغاء</button>
        <button onClick={handleSave} disabled={saving || !name.trim()} className="px-4 py-2 bg-emerald-500 text-white rounded-md text-sm font-medium hover:bg-emerald-600 disabled:opacity-50 transition-colors">
          {saving ? "جاري الحفظ..." : editing ? "حفظ التغييرات" : "إضافة"}
        </button>
      </div>
    </div>
...
function DeliveryHistoryView({ fmt }: { fmt: (c: number) => string }) {
  ...
  if (loading) {
    return <div className="flex justify-center py-10"><div className="w-6 h-6 border-2 border-emerald-500/30 border-t-emerald-500 rounded-full animate-spin" /></div>;
  }
  ...
  return (
    <div className="space-y-3">
      <h2 className="font-semibold text-slate-700">سجل التوصيل ({logs.length})</h2>
      <div className="overflow-x-auto rounded-lg border border-slate-200">
        <table className="w-full text-sm">
          <thead className="bg-slate-50">
            <tr>
              <th className="text-right px-4 py-2.5 text-slate-500 font-medium">العميل</th>
              <th className="text-right px-4 py-2.5 text-slate-500 font-medium">السائق</th>
              <th className="text-right px-4 py-2.5 text-slate-500 font-medium">الحالة</th>
              <th className="text-right px-4 py-2.5 text-slate-500 font-medium">المبلغ</th>
              <th className="text-right px-4 py-2.5 text-slate-500 font-medium">التاريخ</th>
            </tr>
          </thead>
            ...
            {logs.map((log: any) => {
              ...
              return (
                <tr key={log.log_id} className="hover:bg-slate-50">
                  <td className="px-4 py-2.5 font-medium text-slate-700">{log.customer_name || "بدون اسم"}</td>
                  <td className="px-4 py-2.5 text-slate-600">{log.driver_name}</td>
                  <td className="px-4 py-2.5"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${badge.class}`}>{badge.label}</span></td>
                  <td className="px-4 py-2.5 text-slate-600">{fmt(log.total_cents)}</td>
                  <td className="px-4 py-2.5 text-slate-400 text-xs">{formatDate(log.assigned_at)}</td>
                </tr>
              );
            })}
          </tbody>
