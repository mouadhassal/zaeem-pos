...
const TIER_CONFIG: Record<string, { label: string; color: string; icon: string; min_points: number; multiplier: number }> = {
  ...
  SILVER: { label: "فضي", color: "text-slate-600 bg-slate-100 border-slate-300", icon: "🥈", min_points: 500, multiplier: 1.2 },
  ...
  return (
    <div className="p-6 space-y-6 overflow-y-auto h-full" dir="rtl">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-slate-900">برنامج الولاء</h1>
        <button onClick={() => setShowIssue(true)} className="h-10 px-4 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 transition-colors flex items-center gap-2">
        ...
        {TIERS.map((t) => (
          <div key={t} className={`bg-white rounded-2xl p-4 shadow-sm border ${TIER_CONFIG[t].color} border-opacity-30`}>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold text-slate-900">{TIER_CONFIG[t].label}</span>
            </div>
            <p className="text-xs text-slate-400 mt-2">من {TIER_CONFIG[t].min_points} نقطة</p>
            <p className="text-xs text-slate-400">مضاعف: x{TIER_CONFIG[t].multiplier}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2 border-b border-slate-200 pb-2">
        {(["cards", "transactions"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`px-5 py-2 rounded-t-lg font-arabic font-medium text-sm transition-colors ${
            tab === t ? "bg-emerald-600 text-white shadow-sm" : "text-slate-500 hover:text-emerald-600 hover:bg-white"
          }`}>
      ...
      {tab === "cards" && (
        <div className="space-y-4">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ابحث عن عميل أو رقم بطاقة..." className="w-full h-10 pr-10 pl-4 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 font-arabic" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredCards.map((card) => {
              ...
              return (
                <div key={card.id} className="bg-white rounded-2xl shadow-sm overflow-hidden border border-slate-200 hover:shadow-md transition-shadow">
                  <div className="bg-gradient-to-l from-emerald-600 to-emerald-800 p-4 text-white">
                    <div className="flex items-center justify-between">
                  ...
                  <div className="p-4 space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-400 font-arabic">النقاط</span>
                      <span className="font-bold text-emerald-600 font-mono">{card.points}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-400 font-arabic">رقم الجوال</span>
                      <span className="font-mono text-slate-900" dir="ltr">{card.customer_phone}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-400 font-arabic">تاريخ الإصدار</span>
                      <span className="text-slate-500">{card.issued_at.slice(0, 10)}</span>
                    </div>
                    {card.last_used_at && (
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-400 font-arabic">آخر استخدام</span>
                        <span className="text-slate-500">{card.last_used_at.slice(0, 10)}</span>
                      </div>
                    )}
            ...
            {filteredCards.length === 0 && (
              <div className="col-span-full text-center py-12 text-slate-500 font-arabic">
                لا توجد بطاقات ولاء
              </div>
            )}
          </div>
      ...
      {tab === "transactions" && (
        <div className="space-y-4">
          <div className="flex gap-3">
            <select value={txCardFilter} onChange={(e) => setTxCardFilter(e.target.value)} className="h-10 px-4 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {transactions.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id} className="border-b border-slate-200 hover:bg-white">
                      <td className="p-3 font-mono text-slate-500 text-xs">{formatDateTime(tx.created_at)}</td>
                      <td className="p-3">
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-arabic font-medium ${
                          isEarn ? "bg-emerald-100 text-emerald-600" : isRedeem ? "bg-red-100 text-red-600" : "bg-amber-100 text-amber-700"
                        }`}>
                          {isEarn ? "إضافة" : isRedeem ? "استبدال" : tx.type === "ADJUST" ? "تعديل" : "انتهاء صلاحية"}
                        </span>
                      </td>
                      <td className={`p-3 font-mono font-bold ${isEarn ? "text-emerald-600" : "text-red-500"}`}>
                        {isEarn ? "+" : ""}{tx.points}
                      </td>
                ...
                {transactions.length === 0 && (
                  <tr><td colSpan={4} className="p-6 text-center text-slate-500 font-arabic">لا توجد حركات</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {showIssue && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 space-y-4">
            <h2 className="text-lg font-bold text-slate-900 font-arabic">إصدار بطاقة ولاء جديدة</h2>
            <div>
              <label className="block text-sm font-arabic text-slate-900 mb-1">العميل</label>
              <select value={selectedCustomer} onChange={(e) => setSelectedCustomer(e.target.value)} className="w-full h-10 px-4 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={handleIssueCard} disabled={!selectedCustomer} className="flex-1 h-10 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 transition-colors disabled:opacity-40">إصدار البطاقة</button>
              <button onClick={() => setShowIssue(false)} className="px-6 h-10 rounded-xl border border-slate-200 text-slate-500 text-sm font-bold hover:bg-white transition-colors">إلغاء</button>
            </div>
          </div>
        </div>
      )}
