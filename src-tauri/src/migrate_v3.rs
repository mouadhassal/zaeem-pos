...
pub const MIGRATION_R_VERSION: i64 = 22;

/// Marketplace "reorder low-stock ingredients" feature (cross-repo, POS +
/// `apps/marketplace`): the POS's own `ingredients.current_stock` becomes
/// a 5th synced category alongside orders/supplier_payments/operational_costs
/// /staff. Widens `sync_outbox.table_name`'s CHECK to admit `'ingredients'`,
/// same rebuild pattern as v12's supplier-ledger widening and v14's staff
/// widening (SQLite can't ALTER a CHECK in place).
pub fn run_ingredient_sync_migration(conn: &mut Connection, _db_path: &Path) -> Result<(), V3Error> {
    let already: bool = conn
        .query_row("SELECT COUNT(*) > 0 FROM schema_migrations WHERE version = ?1", params![MIGRATION_R_VERSION], |row| row.get(0))
        .unwrap_or(false);
    if already {
        return Ok(());
    }
    ...
    if table_exists(&tx, "sync_outbox")? {
        tx.execute_batch(
            "CREATE TABLE sync_outbox_v22 (
                id TEXT PRIMARY KEY,
                table_name TEXT NOT NULL CHECK(table_name IN ('orders','order_items','payments','supplier_payments','operational_costs','staff','ingredients')),
                row_id TEXT NOT NULL,
                tenant_id TEXT NOT NULL,
                branch_id TEXT NOT NULL,
                payload_json TEXT NOT NULL,
            ...
            INSERT INTO sync_outbox_v22 SELECT * FROM sync_outbox;
            DROP TABLE sync_outbox;
            ALTER TABLE sync_outbox_v22 RENAME TO sync_outbox;
            CREATE INDEX IF NOT EXISTS idx_sync_outbox_due ON sync_outbox(status, next_attempt_at);
            CREATE INDEX IF NOT EXISTS idx_sync_outbox_tenant ON sync_outbox(tenant_id);"
        )?;
    }

    println!("v22_ingredient_sync: sync_outbox table_name CHECK admits 'ingredients'");

    let applied_at = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs() as i64;
    tx.execute(
        "INSERT INTO schema_migrations (version, name, applied_at, checksum) VALUES (?1, ?2, ?3, ?4)",
        params![MIGRATION_R_VERSION, "0022_ingredient_sync", applied_at, "n/a-programmatic"],
    )?;
    tx.commit()?;
    Ok(())
}
...
// 28 and 29 -- NOT 26/27: those two versions are already claimed by
// migrate.rs's own embedded SQL chain (0026_stock_counts.sql,
// 0027_fleet_removal.sql), which shares this exact same
// `schema_migrations` version space (see migrate.rs's own `embedded_
// migrations` doc comment for the collision this bit everyone already
// once, 2026-09-07). 28 is the first free slot past BOTH ceilings.
pub const MIGRATION_V_VERSION: i64 = 28;
...
pub fn run_debtor_credit_limit_migration(conn: &mut Connection, _db_path: &Path) -> Result<(), V3Error> {
    ...
    tx.execute(
        "INSERT INTO schema_migrations (version, name, applied_at, checksum) VALUES (?1, ?2, ?3, ?4)",
        params![MIGRATION_V_VERSION, "0028_debtor_credit_limit", applied_at, "n/a-programmatic"],
    )?;
    tx.commit()?;
    Ok(())
}

pub const MIGRATION_W_VERSION: i64 = 29;
...
pub fn run_menu_item_barcode_tenant_unique_migration(conn: &mut Connection, db_path: &Path) -> Result<(), V3Error> {
    ...
    let result = with_snapshot_protection(conn, db_path, "v29_barcode_unique", |tx| {
        if !table_exists(tx, "menu_items")? {
            ...
            tx.execute(
                "INSERT INTO schema_migrations (version, name, applied_at, checksum) VALUES (?1, ?2, ?3, ?4)",
                params![MIGRATION_W_VERSION, "0029_menu_item_barcode_tenant_unique", applied_at, "n/a-programmatic"],
            )?;
            return Ok(());
        }
